...
Optional arguments:
     ...
     -c:  Control for parallel computation (default 1) -- 0 == run serially,  1 == SGE qsub,
          2 == use PEXEC (localhost),  3 == Apple XGrid, 4 == PBS qsub,  5 == Condor(+fsl_sub)
...
--------------------------------------------------------------------------------------
+Condor support using fsl_sub by Seung-Goo KIM
--------------------------------------------------------------------------------------
...
Optional arguments:
     ...
     -c:  Control for parallel computation (default 1) -- 0 == run serially,  1 == SGE qsub,
    2 == use PEXEC (localhost), 3 == Apple XGrid, 4 == PBS Grid, 5 == Condor(+fsl_sub)
...
- waitForCONDORJobs.sh (only for use with CONDOR)
...
Apple XGrid support by Craig Stark
--------------------------------------------------------------------------------------
+Condor support using using fsl_sub by Seung-Goo KIM
--------------------------------------------------------------------------------------
...
while getopts "c:d:g:i:j:h:m:n:o:p:s:r:t:x:z:" OPT
  ...
  case $OPT in
    ...
    DOQSUB=$OPTARG
    if [[ ${#DOQSUB} -gt 2 ]] ; then
        echo " DOQSUB must be an integer value (0=serial, 1=SGE qsub, 2=try pexec, 3=XGrid, 4=PBS qsub 5=CONDOR) you passed  -c $DOQSUB "
        exit 1
    fi
    ;;
      d) #dimensions
...
if [ ${NINFILES} -eq 0 ]; then
  ...
  if [ ${range} -eq 1 ] && [ ${TDIM} -ne 4 ]; then
    echo "Please provide at least 2 filenames for the template."
    echo "Use `basename $0` -h for help"
    exit 1
  elif [ ${range} -gt 1 ] && [ ${TDIM} -ne 4 ]; then
    echo "This is a multivolume file. Use -d 4"
    echo "Use `basename $0` -h for help"
    exit 1
  elif [ ${range} -gt 1 ] && [ ${TDIM} -eq 4 ]; then
    echo
    echo "--------------------------------------------------------------------------------------"
    ...
    elif [ ${range} -gt ${nfmribins}  ] && [ ${range} -lt 32  ]; then
      for ((i = 0; i < ${nfmribins} ; i++))
      do
        number=$RANDOM
        let "number %= $range"
...
if [ "$RIGID" -eq 1 ]; then
  ...
  if [ $DOQSUB -eq 5 ]; 
    ...
    echo
    echo "--------------------------------------------------------------------------------------"
    echo " Starting ANTS rigid registration on CONDOR cluster. Submitted $count jobs "
    echo "--------------------------------------------------------------------------------------"
...
# fi
  ...
  for IMG in $IMAGESETVARIABLE
    ...
    if [ $DOQSUB -eq 1 ]; then
      ...
      qscript="job_${count}_${i}.sh"
      # Send exe and exe2 to same job file so that they execute in series
      echo $exe  >> ${qscript}
      echo "bash ${qscript}" >> ${joblist}
    fi
  ...
  if [ $DOQSUB -eq 1 ]; then
    ...
    echo
    echo "--------------------------------------------------------------------------------------"
    echo " Starting ANTS registration on CONDOR cluster. Iteration: $itdisplay of $ITERATIONLIMIT"
    echo "--------------------------------------------------------------------------------------"
    # now submit them all together
    qid=`fsl_sub -t ${joblist}`
    # set the priority to the lowest otherwise other people in condor will freak out
  ...
  if [ $DOQSUB -eq 1 ]; then
    ...
    mv job* ${TRANSFORMATIONTYPE}_iteration_${i}
  fi
